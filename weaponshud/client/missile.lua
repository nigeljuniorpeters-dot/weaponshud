...
function Missile_Update()
  ...
  if tgt == 0 or not DoesEntityExist(tgt) then
  ...
  if dist <= (cfg.ProximityFuse or Config.Missile.ProximityFuse) then
    Projectile_ExplodeAt(p)
    Projectile_Clear()
    return
  end
  ...
  -- decoy logic
  if nowMs() >= seeker.decoyUntil then
  ...
  local desiredDir
  if seeker.decoyUntil > nowMs() then
