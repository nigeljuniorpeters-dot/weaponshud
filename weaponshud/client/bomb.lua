...
function Bomb_TryDropDumb(jet)
  local ok, reason = Projectile_CanFire()
...
local function shouldExplode(obj, fromPos, toPos)
  ...
  if GetGameTimer() > noHitUntil then
    local hit, hitPos = Projectile_SweepHitIgnore(Projectile.ownerEnt, fromPos, toPos)
    if hit then return true, hitPos end
  end
...
function Bomb_Update()
  ...
  if Projectile.kind == "bomb_guided" then
    ...
    if tgt ~= 0 and DoesEntityExist(tgt) then
      ...
      Projectile.vel = newDir * Config.Bomb.Guided.Speed
    else
  ...
  local boom, boomPos = shouldExplode(obj, p, newPos)
  if boom then
